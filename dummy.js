...
chrome.runtime.onInstalled.addListener(() => {
   ...
   * Handles requests such as transcript search, search history, etc.
   */
  chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
    try {
      switch (request.action) {
        // Object detection is currently disabled. Uncomment if needed.
        /*
        case "searchObject":
   ...
   * (You can remove this if you no longer use authenticated calls.)
   */
  ...
  /**
   * (Commented out) Handles object detection search by making an API call to the backend.
   * Uncomment this function if you need object detection later.
   */
  ...
  async function fetchFromSPI(endpoint, payload, sendResponse) {
    ...
    try {
      ...
      const requestBody = { ...payload, videoId: finalVideoId };
      const response = await fetch(apiUrl, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
