...
class TranscriptUtilsTestSuite(unittest.TestCase):
    ...
    @pytest.mark.skip(
        reason="Have to fix cookies first. Avoiding blocking the development."
    )
    def test_create_transcript(self):
        t_utils = transcriptUtils()
        model = whisper.load_model(
            "tiny"
        )  # testing with smaller model for faster test run
        model.to("cpu")
        self.file_path = t_utils.create_transcript(
            self.no_transcript_url, "test_transcript", model
        )

        self.assertTrue(self.file_path)
        self.assertTrue(
            os.path.exists("temp/subtitles/test_transcript.vtt"),
